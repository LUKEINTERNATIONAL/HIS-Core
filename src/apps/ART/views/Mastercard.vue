...
export default defineComponent({
    ...
    methods: {
        ...
        async getPatientCardInfo(patient: Patientservice) {
            ...
            let dateOfTest = await ObservationService.getAll(this.patientId, 'Confirmatory HIV test date');
            dateOfTest = toStandardHisDisplayFormat(dateOfTest[0].value_datetime);
            const placeOfTest = await ObservationService.getFirstValueText(this.patientId, 'Confirmatory HIV test location');

            let startDate = await ObservationService.getAll(this.patientId, 'Date ART started');
            ...
            const TI = await ObservationService.getFirstValueCoded(this.patientId, 'Ever received ART');
            const agreesToFollowUp = await ObservationService.getFirstValueCoded(this.patientId, 'Agrees to followup');
            const reasonForStarting = await ObservationService.getFirstValueCoded(this.patientId, 'Reason for ART eligibility');
            ...
            const obj: Option[] = [
                ...
                {label: 'Date + place of HIV test', value: `${dateOfTest} ${placeOfTest}`},
        ...
        async getInitial(concept: string) {
            const initialObs = await ObservationService.getAll(this.patientId, concept);
